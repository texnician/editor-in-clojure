...
(defn gen-cpp-component [comp-key]
  (let [group (STGroupFile. "st/cpp/component.stg")
        ...
        class_st (.getInstanceOf group "cpp_class")]
    (doto st
      ...
      (.add "body" (doto class_st
                     (.add "name" (cpp-component-name :base))
                     (.add "bases" "IComponent")
                     (.add "bases" "Object")
                     (.add "statements" "int a;")
                     (.add "statements" "char b;")
                     (.render))))
    (spit "tmp.h" (.render st))))

(let [group (STGroupFile. "st/cpp/class.stg")
      class_st (.getInstanceOf (STGroupFile. "st/cpp/class.stg") "cpp_class")]
  (doto class_st
    (.add "name" (cpp-component-name :base))
    (.add "bases" "IComponent")
    (.add "bases" "Object")
